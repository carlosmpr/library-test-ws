...
4. **`extractFiles(items)`**: Asynchronously extracts files from the data transfer items. It checks if the item is a file or directory and calls `isValidFile` to validate the file.

5. **`traverseFileTree(item, filesArray)`**: Recursively traverses a file tree, handling files and directories. It uses promises for asynchronous operations.

6. **`isValidFile(file)`**: Checks if the file has a valid extension by comparing it with the `allowedExtensions` array.

7. **`handleFileSelect(event)`**: Handles the file selection event from the file input field.

8. **`validateAndSetFiles(files)`**: Validates the selected files against allowed extensions and maximum file count. It updates the `selectedFiles` array accordingly.
...
This code snippet provides a basic file upload functionality with drag and drop support, file validation, and error handling in a Svelte component.
