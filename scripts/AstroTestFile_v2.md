...
### Dependencies
- The code imports `useState` from React, which is a React Hook used to manage state in functional components.
- The code also imports icons from the `@heroicons/react` library for displaying icons in the UI.
...
7. `validateAndSetFiles`: Validates selected files against allowed extensions and the maximum file limit.

### Usage
The `FileDropZone` component renders a drop zone where users can drag and drop files or click to select files. It displays the selected files with their names and icons. If an invalid file type is selected or the maximum file limit is exceeded, an error message is displayed.
