...
- The code also imports icons from the `@heroicons/react` library to display icons for files and folders.
...
### Usage
- The component renders a styled drop zone where users can drag and drop files or click to select files.
- When files are selected or dropped, they are validated against allowed extensions and the maximum file limit.
- If the files are valid, they are added to the `selectedFiles` state array and displayed with their names and icons.
- If there are errors (e.g., invalid file type or exceeding the file limit), an error message is displayed.
